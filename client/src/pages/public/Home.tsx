  ...
  useEffect(() => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/api/cms/banners`)
      ...
      .then((data) => {
        ...
        setHeroSlides(
          activeBanners.filter((b: any) => !b.type || b.type === "hero"),
        );
      })
      .catch(console.error);
      ...
      <section className="border-t border-slate-100 bg-white">
        <div className="mx-auto grid max-w-7xl gap-4 px-4 py-6 sm:px-6 lg:grid-cols-12 lg:px-8">
          ...
          <aside className="self-start overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm transition-all duration-300 hover:shadow-md lg:col-span-3">
            <div className="flex items-center gap-2.5 bg-gradient-to-r from-orange-500 to-orange-300 px-4 py-4 text-white">
              ...
              {categoryMenu.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      {category.image ? (
                        ...
                        <span
                          className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${style.color} transition-all duration-300 group-hover:scale-110 group-hover:shadow-sm`}
                        >
                          <Icon className="h-4.5 w-4.5" />
                        </span>
                      )}
                      <span className="min-w-0 flex-1 truncate transition-colors duration-200 group-hover:text-rose-700">
                        {category.name}
                      </span>
                ...
                <div className="space-y-2.5 p-1.5">
                  {[...Array(5)].map((_, index) => (
                    <div
                      key={index}
                      className="h-10 rounded-xl bg-slate-50 animate-pulse"
                    />
                  ))}
                </div>
              )}
                ...
                heroSlides.map((slide, index) => (
                  ...
                  >
                    <img
                      src={slide.image}
                      alt={slide.title}
                      className="absolute inset-0 h-full w-full object-cover opacity-85 transition-transform duration-[8000ms] ease-out group-hover/slider:scale-105"
                    />
                    ...
                    <div className="absolute left-8 top-1/2 z-10 flex max-w-[280px] -translate-y-1/2 flex-col items-start text-left text-white sm:left-12 sm:max-w-[360px]">
                      ...
                      <h1 className="text-2xl font-extrabold leading-tight text-white drop-shadow-md sm:text-3.5xl lg:text-4xl">
                        {slide.title}
                      </h1>
                      {slide.subtitle && (
                        <p className="mt-2 line-clamp-2 text-sm font-medium text-slate-200 drop-shadow">
                          {slide.subtitle}
                        </p>
                      )}
            ...
            <div className="mt-4 grid overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm sm:grid-cols-4">
              {quickBenefits.map((benefit) => {
                ...
                return (
                  <div
                    key={benefit.label}
                    className="group/benefit flex min-h-14 items-center justify-center gap-2.5 border-b border-slate-100 px-3 py-3 text-center text-xs font-bold text-slate-700 transition-all duration-300 hover:bg-slate-50 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0"
                  >
              ...
              if (hasImage) {
                ...
                return (
                  ...
                  >
                    ...
                    {Icon && (
                      <Icon
                        className={`absolute right-4 top-4 h-12 w-12 ${promo.iconColor} opacity-20 transition-all duration-500 group-hover:scale-110 group-hover:-translate-y-1 group-hover:rotate-6`}
                      />
                    )}
          ...
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-rose-600 via-rose-500 to-orange-500 shadow-xl shadow-rose-100/50 transition-all duration-300 hover:shadow-rose-200/60">
            ...
            <div className="flex flex-col md:flex-row md:items-stretch">
              ...
              <div className="relative flex flex-col items-center justify-center bg-slate-950/20 px-8 py-8 text-center md:w-1/4 md:border-r-2 md:border-dashed md:border-white/20">
                ...
                <span className="text-[10px] font-extrabold uppercase tracking-widest text-rose-100">
                  ƯU ĐÃI ĐỘC QUYỀN
                </span>
                <p className="mt-1.5 text-4xl font-black text-white sm:text-5xl">
                  {pinnedCoupon.type === "percentage" &&
                    `${pinnedCoupon.value}%`}
                  {pinnedCoupon.type === "fixed" &&
                    `${(pinnedCoupon.value / 1000).toLocaleString("vi-VN")}K`}
                  {pinnedCoupon.type === "shipping" && "FREE"}
                </p>
                <p className="mt-1 text-xs font-semibold text-rose-50/90 uppercase tracking-wide">
                  {pinnedCoupon.type === "shipping"
                    ? "Vận chuyển"
                    : "Giảm giá trực tiếp"}
                </p>
              </div>
                ...
                <div className="flex-1 space-y-4">
                  <div className="space-y-2">
                    <h3 className="text-xl font-extrabold sm:text-2xl">
                      Mã giảm giá đặc biệt dành cho bạn
                    </h3>
                    <p className="text-sm text-rose-50/90 leading-relaxed max-w-xl">
                      {pinnedCoupon.type === "percentage" &&
                        `Nhập mã bên dưới để được giảm ngay ${pinnedCoupon.value}% tổng giá trị đơn hàng.`}
                      {pinnedCoupon.type === "fixed" &&
                        `Nhập mã bên dưới để được giảm ngay ${pinnedCoupon.value.toLocaleString("vi-VN")}đ.`}
                      {pinnedCoupon.type === "shipping" &&
                        `Nhập mã bên dưới để được miễn phí vận chuyển cho đơn hàng của bạn.`}
                      {pinnedCoupon.minOrder > 0 &&
                        ` Áp dụng cho đơn hàng tối thiểu từ ${pinnedCoupon.minOrder.toLocaleString("vi-VN")}đ.`}
                    </p>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <div className="group relative flex items-center overflow-hidden rounded-xl border border-white/20 bg-white/10 p-1 pl-4 pr-3 backdrop-blur-sm">
                      <span className="text-xs font-bold uppercase tracking-wider text-rose-200 mr-3">
                        Mã:
                      </span>
                      ...
                      <button
                        onClick={() => {
                          ...
                          showToast(
                            "Đã sao chép mã giảm giá thành công!",
                            "success",
                          );
                        }}
                      ...
                      <div className="flex items-center gap-1.5 text-xs font-black">
                        {timeLeft.days > 0 && (
                          <div className="flex flex-col items-center gap-1">
                            ...
                            <span className="text-[8px] text-rose-200 font-bold uppercase tracking-wider">
                              ngày
                            </span>
                          </div>
                        )}
                        ...
                        <span className="text-rose-200/80 mb-3.5 font-bold">
                          :
                        </span>
        ...
        <div className="mb-8 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between border-b border-slate-100 pb-4">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl bg-gradient-to-r from-slate-900 via-slate-800 to-rose-900 bg-clip-text">
              Sản phẩm nổi bật ABC
            </h2>
          </div>
          <Link
            to="/products"
            className="group inline-flex items-center gap-1 text-sm font-extrabold text-rose-600 transition-all hover:text-rose-700"
          >
            Xem tất cả{" "}
          ...
          {featuredProducts.length > 0 ? (
            featuredProducts.map((product) => (
              <ProductCard
                ...
                badge={
                  product.compareAtPrice > product.price ? "Sale" : undefined
                }
                inStock={
                  Number(product.totalVariantStock ?? product.stock ?? 0) > 0
                }
              />
            ))
          ) : (
