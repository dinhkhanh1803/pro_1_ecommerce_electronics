...
const EXPRESS_ADDITIONAL = SHIPPING_FEE;
...
export function Checkout() {
  ...
  const baseShipping =
    freeShipping || subtotal >= FREE_SHIP_THRESHOLD ? 0 : SHIPPING_BASE;
  const shipping =
    baseShipping + (deliveryMethod === "express" ? EXPRESS_ADDITIONAL : 0);
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      if (!userProfile?.phone || userProfile.phone !== formData.phone)
      ...
      if (res.ok) {
        ...
        if (paymentMethod === "vnpay") {
          ...
          if (vnpayData.paymentUrl) {
            ...
            alert(
              vnpayData.message || "Không thể tạo liên kết thanh toán VNPay.",
            );
            return;
          }
        }
        ...
        if (paymentMethod === "momo") {
          const momoRes = await fetch(
            `${import.meta.env.VITE_API_URL}/api/payment/create_momo_payment_url`,
            {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
          ...
          if (momoData.paymentUrl) {
            ...
            alert(
              momoData.message || "Không thể tạo liên kết thanh toán MoMo.",
            );
            return;
          }
        }
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
              ...
              <div className="space-y-3 mb-6 pb-6 border-b border-gray-200">
                ...
                <div className="flex justify-between text-gray-600 text-sm">
                  <span>
                    Vận chuyển (
                    {deliveryMethod === "express" ? "Nhanh" : "Thường"})
                  </span>
                  <span
                    className={
              ...
              <p className="text-xs text-gray-500 text-center mt-4">
                Bằng cách đặt hàng, bạn đồng ý với Điều khoản & Điều kiện của
                chúng tôi
              </p>
            </div>
          </div>
